...
class HumorDiffusionTransformer(nn.Module):
    ...
    def sample_step(self, past_in, t_in=None, use_mean=False, z=None, return_prior=False, return_z=False):
        ...
        z_hat = self.diffusion.p_sample_loop(
            self.diffusion_model.forward_with_cfg, z_noise.shape, None,  clip_denoised=False, model_kwargs=model_kwargs, progress=True, device=past_in.device
        )
    ...
    def infer_global_seq(self, global_seq, full_forward_pass=False):
        ...
        if full_forward_pass:
            raise NotImplementedError('Full forward pass not implemented yet!')
        ...
        for t in range(T-1):
            ...
            if full_forward_pass:
                raise NotImplementedError('Full forward pass not implemented yet!')
                # x_pred_dict = self(x_past, x_t) # NOTE: call forward.
        ...
        latent_states = torch.stack(latent_states, axis=1) # TODO: axis=1 or 2?
