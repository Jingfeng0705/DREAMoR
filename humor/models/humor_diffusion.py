...
class HumorModel(nn.Module):
    ...
    def __init__(self,  diffusion_cfg,
        ...
        self.decoder = MLP(layers=layer_list,
                            nonlinearity=nn.ReLU, 
                            use_gn=True,
                            skip_input_idx=None # skip connect the latent to every layer, since latent is the only input
                        )

        # We don't need the prior anymore
    ...
    def forward(self, x_past, x_t):
        ...
        Input:
        - x_past (B x steps_in x D), steps_in = 1
        - x_t    (B x steps_out x D), steps_out = 1
